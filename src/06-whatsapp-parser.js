...
export function parseWhatsAppMessage(message) {
  // Your code here
  if (typeof message !== "string" || message.trim() === "" || !message.includes(", ") || !message.includes(" - ") || !message.includes(": ")) return null;
  ...
  const date = message.substring(0, dateEndIndex).trim();
  const time = message.substring(dateEndIndex + 2, timeEndIndex).trim();
  const sender = message.substring(timeEndIndex + 3, senderEndIndex).trim();
