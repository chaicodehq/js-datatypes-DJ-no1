...
export function iplAuctionSummary(team, players) {
  ...
  const costlyPlayer = players.reduce((costly, player)=> player.price > costly.price ? player : costly, players[0]);
  ...
  const byRole = players.reduce((count, player) => {
